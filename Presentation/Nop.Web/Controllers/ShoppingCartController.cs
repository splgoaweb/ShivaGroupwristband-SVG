...
namespace Nop.Web.Controllers
{
    ...
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        protected virtual IActionResult GetProductToCartDetails(List<string> addToCartWarnings, ShoppingCartType cartType,
            Product product, IFormCollection form)
        {
            var openaddtocartpopuphtml = _shoppingCartSettings.MiniShoppingCartEnabled
                            ? RenderViewComponentToString("AddToCartPopup", new { isErrorNotification = addToCartWarnings.Any() ? true : false, addToCartWarnings = addToCartWarnings, form = form, productId = product.Id, shoppingCartTypeId = cartType == ShoppingCartType.Wishlist ? (int)ShoppingCartType.Wishlist : (int)ShoppingCartType.ShoppingCart })
                            : string.Empty;
        ...
        public virtual IActionResult AddProductToCart_Details(int productId, int shoppingCartTypeId, IFormCollection form)
        {
            ...
            //return result
            return GetProductToCartDetails(addToCartWarnings, cartType, product, form);
        }
            ...
            bool loadPicture, IFormCollection form)
        {
            ...
            var quantity = _productAttributeParser.ParseEnteredQuantity(product, form);
            quantity = RoundedToHundred(Convert.ToDecimal(quantity));
            // custom code end
            //price
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual IActionResult AddProductToCartWithMultipleQuantity_Details(int productId, int shoppingCartTypeId, IFormCollection form)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
            {
            ...
            var productAttributes = _productAttributeService.GetProductAttributeMappingsByProductId(product.Id);
            var colorsquarelist = new List<string>();
            var totalquantity = 0;
                    ...
                    case AttributeControlType.ColorSquares:
                        {
                            ...
                            var colorSqureList = form.Where(x => x.Key.Contains(controlId + "_")).ToList();
                            //var ctrlAttributes = form[controlId];
                            //if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            //{
                            //    var selectedAttributeId = int.Parse(ctrlAttributes);

                            foreach (var item in colorSqureList)
                            {
                                if (!string.IsNullOrEmpty(item.Value))
                                {
                                    colorsquarelist.Add(item.Key.Replace("_qty", ""));
                                    totalquantity += int.Parse(item.Value);
                                }
                                else
                                {
                                    totalquantity += 0;
            ...
            if (colorsquarelist.Count > 0)
            {
                var addToCartWarnings = new List<string>();

                foreach (var item in colorsquarelist)
                {
                    //customer entered price
                    var customerEnteredPriceConverted = _productAttributeParser.ParseCustomerEnteredPrice(product, form);

                    //entered quantity
                    var quantity = _productAttributeParser.CustomParseEnteredQuantity(product, form, item);

                    //product and gift card attributes
                    var attributes = _productAttributeParser.CustomParseProductAttributes(product, form, addToCartWarnings, item);
                ...
                //return result
                return GetProductToCartDetails(addToCartWarnings, shoppingCartTypeId == 1 ? ShoppingCartType.ShoppingCart : ShoppingCartType.Wishlist, product, form);
            }
            else
            {
                var addToCartWarnings = new List<string>();
                ...
                SaveItem(updatecartitem, addToCartWarnings, product, cartType, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, totalquantity);
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual IActionResult ProductDetails_QtyAttributeChange(int productId, bool validateAttributeConditions,
            bool loadPicture, IFormCollection form)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
            ...
            int totalquantity = 0;
            var quntitylist = new List<object>();
            foreach (var attribute in productAttributes)
            {
                var controlId = $"{NopCatalogDefaults.ProductAttributePrefix}{attribute.Id}";
                switch (attribute.AttributeControlType)
                    ...
                    case AttributeControlType.ColorSquares:
                        {
                            ...
                            foreach (var item in colorSqureList)
                            {
                                if (!string.IsNullOrEmpty(item.Value))
                                {
                                    var quantity = RoundedToHundred(Convert.ToDecimal(int.Parse(item.Value)));
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices) && !product.CustomerEntersPrice)
            {
                ...
                var finalPrice = _shoppingCartService.GetUnitPrice(product,
                    _workContext.CurrentCustomer,
                    ShoppingCartType.ShoppingCart,
                   (totalquantity != 0 ? totalquantity : 1), attributeXml, 0,
                    rentalStartDate, rentalEndDate,
                    true, out var _, out _);
                var finalPriceWithDiscountBase = _taxService.GetProductPrice(product, finalPrice, out var _);
                var finalPriceWithDiscount = _currencyService.ConvertFromPrimaryStoreCurrency(finalPriceWithDiscountBase, _workContext.WorkingCurrency);

                var totalpricewithdiscount = finalPriceWithDiscount * (totalquantity != 0 ? totalquantity : 1);
                price = _priceFormatter.FormatPrice(totalpricewithdiscount);
                unitprice = _priceFormatter.FormatPrice(finalPriceWithDiscount);
                //basepricepangv = _priceFormatter.FormatBasePrice(product, finalPriceWithDiscountBase, totalWeight);
            }
        ...
        public int RoundedToHundred(decimal value)
        {
            int result = (int)Math.Ceiling(value / 100);
            if (value > 0 && result == 0)
            {
                result = 1;
            }
