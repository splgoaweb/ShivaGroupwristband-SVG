...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual IList<ProductDetailsModel.TierPriceModel> PrepareProductTierPriceModels(Product product)
        {
            ...
            var productcategory = _categoryService.GetProductCategoriesByProductId(product.Id);
            if (productcategory == null || productcategory.Count == 0)
                return new List<ProductDetailsModel.TierPriceModel>();


            var category = _categoryService.GetCategoryById(productcategory.FirstOrDefault().CategoryId);
            if (category == null)
                return new List<ProductDetailsModel.TierPriceModel>();


            var enableaggrigation = category.EnableAggregation.HasValue ? category.EnableAggregation.Value : false;

            if (!enableaggrigation)
                return new List<ProductDetailsModel.TierPriceModel>();

            var model = _tierpriceService.GetCategoryTierPrices(category, _workContext.CurrentCustomer, _storeContext.CurrentStore.Id)
                   .Select(tierPrice =>
                   {
                       //var priceBase = _taxService.GetProductPrice(product, _priceCalculationService.GetFinalPrice(product,
                       //    _workContext.CurrentCustomer, decimal.Zero, _catalogSettings.DisplayTierPricesWithDiscounts,
                       ...
                       var price = _currencyService.ConvertFromPrimaryStoreCurrency(tierPrice.Price, _workContext.WorkingCurrency);
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                Enableaggrigation = PrepareEnableAggrigation(product)
        ...
        // custom code start
        protected virtual bool PrepareEnableAggrigation(Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
