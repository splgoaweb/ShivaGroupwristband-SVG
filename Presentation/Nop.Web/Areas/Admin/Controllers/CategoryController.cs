...
using Nop.Web.Areas.Admin.CustomCode.Factories;
...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CategoryController : BaseAdminController
    {
        ...
        // custom code start
        private readonly ITirePriceModelFactory _tirePriceModelFactory;
        ...
        public virtual IActionResult TierPriceCreatePopup(int categoryId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
            ...
            //prepare model
            var model = _tirePriceModelFactory.PrepareTierPriceModel(new TierPriceModel(), category, null);
        ...
        public virtual IActionResult TierPriceEditPopup(int id)
        {
            ...
            var tierPrice = _tierpriceService.GetTierPriceById(id);
            if (tierPrice == null)
                return RedirectToAction("List", "Category");

            //try to get a category with the specified id
            var category = _categoryService.GetCategoryById((int)tierPrice.CategoryId)
                ?? throw new ArgumentException("No category found with the specified id");


            //prepare model
            var model = _tirePriceModelFactory.PrepareTierPriceModel(null, category, tierPrice);
        ...
        public virtual IActionResult TierPriceEditPopup(TierPriceModel model)
        {
            ...
            //try to get a tier price with the specified id
            var tierPrice = _tierpriceService.GetTierPriceById(model.Id);
            if (tierPrice == null)
                return RedirectToAction("List", "Category");
            ...
            if (ModelState.IsValid)
            {
                ...
                tierPrice.CategoryId = category.Id;
                tierPrice.CustomerRoleId = model.CustomerRoleId > 0 ? model.CustomerRoleId : (int?)null;
                _tierpriceService.UpdateTierPrice(tierPrice);
            ...
            //prepare model
            model = _tirePriceModelFactory.PrepareTierPriceModel(model, category, tierPrice, true);
        ...
        public virtual IActionResult TierPriceDelete(int id)
        {
            ...
            _tierpriceService.DeleteTierPrice(tierPrice);
