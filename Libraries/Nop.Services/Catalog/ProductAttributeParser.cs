...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductAttributeParser : IProductAttributeParser
    {
        ...
        public virtual string CustomParseProductAttributes(Product product, IFormCollection form, List<string> errors, string colourSqureId)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            if (form == null)
            ...
            //product attributes
            var attributesXml = CustomGetProductAttributesXml(product, form, errors, colourSqureId);
        ...
        protected virtual string CustomGetProductAttributesXml(Product product, IFormCollection form, List<string> errors, string colourSqureId)
        {
            var attributesXml = string.Empty;
            var productAttributes = _productAttributeService.GetProductAttributeMappingsByProductId(product.Id);
            foreach (var attribute in productAttributes)
                    ...
                    case AttributeControlType.ImageSquares:
                        {
                            ...
                            {
                                var selectedAttributeId = int.Parse(colourSqureId.Split('_')[3]);
                                if (selectedAttributeId > 0)
                                {
                                    //get quantity entered by customer
                                    var quantity = 1;
        ...
        public virtual int CustomParseEnteredQuantity(Product product, IFormCollection form, string colourSqureId)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            if (form == null)
            ...
            foreach (var formKey in form.Keys)
            {
                if (formKey.Equals($"{colourSqureId}_qty", StringComparison.InvariantCultureIgnoreCase))
                {
                    int.TryParse(form[formKey], out quantity);
                    break;
                }
