...
namespace Nop.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
            ...
            out List<Discount> appliedDiscounts)
        {
            ...
            // custom code stat
            //var tierpricecalculation = new FinalTierPrice();
            //var currentPageUrl = _webHelper.GetThisPageUrl(true);
            //if (currentPageUrl.Contains("ProductDetails_QtyAttributeChange"))
            //{
            //    tierpricecalculation = _tierpriceService.GetTierPricesModelForTotalQuanity(product.Id, quantity);
            //}
            //else
            //{
            //    if (currentPageUrl.Contains("cart") || currentPageUrl.Contains("checkout"))
            //    {
            //        tierpricecalculation = _tierpriceService.GetTierPricesModel(product.Id, customer, _storeContext.CurrentStore.Id);
            //    }
            //}

            var tierpricecalculation = _tierpriceService.GetTierPricesModel(product.Id, customer, _storeContext.CurrentStore.Id, quantity);
            ...
            //otherwise, its can cause memory leaks (to store all possible date period combinations)
            ...
            (rezPrice, discountAmount, appliedDiscounts) = _staticCacheManager.Get(cacheKey, () =>
            {
                ...
                //tier prices
                if (tierpricecalculation.EnableAggregation == false)
                {
                ...
                {
                    var tierPrice = _tierpriceService.GetPreferredTierPrice(tierpricecalculation.CategoryId, customer, _storeContext.CurrentStore.Id, tierpricecalculation.TotalQuantity);
                    if (tierPrice != null)
                        price = tierPrice.Price;
                }
